    ...
    if request.method == "POST":
        ...
        if not option:
            ...
            if option == "Title":
                query = db.execute(
                    text(
                        "SELECT Isbn , title, Author, Year From Book WHERE title LIKE :title"
                    ),
                    {"title": foo},
                )
                row = query.fetchall()
                ...
                query = db.execute(
                    text(
                        "SELECT Isbn , title, Author, Year From Book WHERE isbn LIKE :isbn"
                    ),
                    {"isbn": foo},
                )
                row = query.fetchall()
                ...
                query = db.execute(
                    text(
                        "SELECT Isbn , title, Author, Year From Book WHERE year LIKE :year LIMIT 10"
                    ),
                    {"year": foo},
                )
                row = query.fetchall()
...
#!la parte de la ruta la hice asi
...
def api(isbn):
    ...
    if rques.status_code != 200:
        ...
        """
        SELECT COUNT(*) as Cantidad, ROUND(AVG(stars)) as Promedio FROM reviews
        WHERE books_id = '0375508325'
        """
        xd  = db.execute(text("SELECT COUNT(*) as Cantidad, ROUND(AVG(stars)) as Promedio FROM reviews WHERE books_id = :isbn"),{"isbn":isbn}).fetchone()
...
def book(isbn):
    ...
    query_books = db.execute(
        text("SELECT *FROM Book WHERE isbn = :isbn"), {"isbn": isbn}
    ).fetchone()
    # Se Muestran los comentarios a ese libro tomando como parametro el id del libro
    query_coment = db.execute(
        text("SELECT *FROM reviews JOIN Book ON books_Id = Isbn WHERE Isbn = :Isbn"),
        {"Isbn": isbn},
    )
    rows = query_coment.fetchall()
    verificar = db.execute(
        text(
            "SELECT stars FROM reviews WHERE username =:username AND books_Id = :book_Id"
        ),
        {"username": session["username"], "book_Id": isbn},
    ).fetchall()
    ...
    for book in book_info["items"]:
        if (
            ...
            or "raitingsCount" in book["volumeInfo"]
            and "imageLinks" in book["volumeInfo"]
        ):
    ...
    if request.method == "POST":
        ...
        verificar = db.execute(
            text("SELECT *From reviews WHERE username =:username"),
        ...
        if not coment:
            ...
            public_coment = db.execute(
                text(
                    "INSERT INTO reviews (username,comments, stars,books_id) VALUES(:username,:comments,:stars,:book_id)"
                ),
    ...
    if puntuacion is not None:
        ...
        while x <= puntuacion:
            ...
            return render_template(
                ...
                star=star,
                coment_stars=coment_stars,
            )  # Se muestra el resultado del libro buscad
    if len(dato) == 0:
        print(dato)
...
def register():
    ...
    if request.method == "POST":
        ...
        query = db.execute(
            text("SELECT *FROM usser WHERE username = :username"), {"username": user}
        ).fetchone()
        if query == None:
            hash = generate_password_hash(passw)
            db.execute(
                text(
                    "INSERT INTO usser (username, password) VALUES(:username, :password)"
                ),
